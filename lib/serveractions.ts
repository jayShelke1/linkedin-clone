...
export const createPostAction = async(inputText:string, selectedFile:string)=>{
    ...
    const userDatabase :IUser={
        firstName:user.firstName || "Blaze",
        lastName:user.lastName || "Fury",
        userId:user.id,
...
// gety all post using server actions
...
export const createCommentAction = async (postId:string, formData:FormData)=>{
    try{
        ...
        const userDatabase:IUser={
            firstName:user.firstName || "BornWithFire",
            lastName:user.lastName || "Ignite",
            userId:user.id,
            profilePhoto:user.imageUrl
        }
